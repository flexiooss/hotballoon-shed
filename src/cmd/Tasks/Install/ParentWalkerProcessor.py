        ...
        if hb_package is not None:
            ...
            if hb_package.config().has_parent_external():
                ...
                if self.parents.get(hb_package.config().parent_name()):
                    ...
                    if hb_package.config().has_parent_version() and self.parents.get(
                            hb_package.config().parent_name()) is not None and self.parents.get(
                            hb_package.config().parent_name()) != hb_package.config().parent_version():
                        raise ImportError(
